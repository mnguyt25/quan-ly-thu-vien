...
def open_login_window():
    ...
    app.iconbitmap("project/icon.ico") 
    app.resizable(False, False) # không cho resize

    # Hiển thị ảnh bên trái
    side_img_data = Image.open("project/side-img.png")
    ...
    def login():
        ...
        result = db.fetch_one("SELECT username, role FROM users WHERE username = %s AND password = %s", (username, password))
        if result:
            user_name = result[0]  # ID người dùng
            ...
            def after_login():
                ...
                if role == "admin":
                    ...
                    open_user_interface(user_name)  # ✅ truyền user_id vào đây
